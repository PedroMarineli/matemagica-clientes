...
const classId = routeId ? parseInt(routeId as string) : null
...
onMounted(async () => {
    try {
        ...
        if (classId !== null) {
            studentsClass.value = students.value.filter(
                (user: IUsers) => user.classroom_id === classId
            )
        } else {
            console.warn("Class ID não encontrado na rota. Não foi possível filtrar os alunos")
        }
    } catch(error) {
    ...
    <div class="w-full h-full flex md:justify-between lg:flex-row flex-1" :class="{'flex-col-reverse md:h-min': showRegisterForm || showMaintainForm }">
        <div class="flex flex-col items-center gap-5 py-5 lg:py-10" :class="{ 'px-20 xl:px-56 w-full': !showRegisterForm && !showMaintainForm, 'px-20 lg:w-2/3': showRegisterForm || showMaintainForm }">
            <div class="card w-full flex items-center justify-between">
                <div>
                    <h1>Alunos</h1>
                    <p>Lista com seus alunos da sala {{ verifyClass(classId!) }}</p>
                </div>
            ...
            <div class="card w-full grid overflow-x-auto justify-center">
                <table class="table">
                    ...
                    <tbody v-if="!studentsClass || studentsClass.length === 0">
                        <tr>
                            <td colspan="10">Nenhum aluno cadastrado nessa sala</td>
                        </tr>
                    </tbody>
                    <tbody v-else v-for="student in studentsClass">
